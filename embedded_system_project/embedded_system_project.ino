...
#define DEBUG_LED1        36   // LED for debugging
#define DEBUG_LED2        35   // LED for debugging
...
void loop() {
  ...
  float rawMic = analogRead(MIC_PIN);

  Serial.print("Raw mic input: ");
  Serial.println(rawMic / 4096 * 3.3);
...
float applyBiquad(float input, const float coeffs[5], float state[2]) {
...
void listenForVoice() {
    const int listenDuration = 5000;  // Listen for 5 seconds
    unsigned long startTime = millis();
    unsigned long currentTime;
    ...
    while ((currentTime = millis()) - startTime < listenDuration) {
        ...
        float micVoltage = micRaw * (3.3 / 4096.0);
